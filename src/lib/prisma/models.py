...
def normalize_datetime_field_value(value: Any, provider: str) -> Any:
    parsed: Any = value
    ...
    normalized = parsed.astimezone(timezone.utc) if parsed.tzinfo else parsed
    if provider == "sqlite":
...
DbNull = _PrismaJsonNull("DbNull")
JsonNull = _PrismaJsonNull("JsonNull")
AnyNull = _PrismaJsonNull("AnyNull")
JsonValue = Any


def is_db_null(value: Any) -> bool:
...
def is_any_null(value: Any) -> bool:
    return value is AnyNull


def normalize_count_selection(option: Any) -> Dict[str, bool]:
    if option is True:
        return {"_all": True}

    if isinstance(option, dict):
        nested = option.get("select")
        if isinstance(nested, dict):
            return {str(key): bool(value) for key, value in nested.items() if value}
        return {str(key): bool(value) for key, value in option.items() if key != "select" and value}
...
class UserSelect(TypedDict, total=False):
    ...
    _count: Union[bool, Dict[str, Any]]
...
class User:
    ...
    _count: Optional[Dict[str, Any]] = None
    ...
    def to_dict(
        ...
        for key in fields_to_include:
            ...
            if isinstance(val, datetime):
                ...
                data[key] = val.value
            elif is_db_null(val) or is_json_null(val) or is_any_null(val):
                data[key] = None
            else:
                data[key] = val
        ...
        if include:
            ...
            count_val = include.get("_count")
            if count_val:
                count_selection = normalize_count_selection(count_val)
                count_data = self._count or {}
                if count_val is True:
                    data["_count"] = dict(count_data)
                else:
                    data["_count"] = {
                        key: count_data.get(key, 0)
                        for key in count_selection.keys()
                    }
        ...
        if select:
            ...
            if sel_val and "userRole" not in data:
                if isinstance(sel_val, dict):
                    ...
                    data["userRole"] = self.userRole.to_dict() if self.userRole else None
            count_val = select.get("_count")
            if count_val and "_count" not in data:
                count_selection = normalize_count_selection(count_val)
                count_data = self._count or {}
                if count_val is True:
                    data["_count"] = dict(count_data)
