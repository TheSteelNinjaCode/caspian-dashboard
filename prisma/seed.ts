...
async function createPrismaClient(): Promise<{
  ...
  if (provider === "postgresql") {
    ...
    const { parse, toClientConfig } = await import("pg-connection-string");
    const postgresConfig = toClientConfig(
      parse(databaseUrl, { useLibpqCompat: true }),
    );
    const adapter = new PrismaPg(postgresConfig);
    const prisma = new PrismaClient({ adapter });
