      ...
      <table class="w-full min-w-4xl text-left">
        ...
        <tbody>
          ...
          <tr class="border-b border-border last:border-b-0">
            ...
            <td class="px-4 py-3"><div class="flex justify-end"><x-button type="button" variant="outline" size="icon-sm" class="rounded-r-none border-r-0" aria-label="Edit {{ user.name }}" onclick='openEditDialog({{ user | json }})'><x-pencil /></x-button><x-button type="button" variant="destructive" size="icon-sm" class="rounded-l-none" aria-label="Delete {{ user.name }}"><x-trash2 /></x-button></div></td>
          </tr>
          {% else %}
